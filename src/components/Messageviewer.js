...
import { addDoc, collection, doc, getDoc } from 'firebase/firestore'; // Import getDoc
...
export default function Message() {
  ...
  const handleOpen = async () => {
    setOpen(true);
    await fetchMessage(); // Fetch the received message when opening the modal
  };
  ...
  const fetchMessage = async () => {
    ...
    const messageRef = collection(userDoc, "Inbox");
    const snapshot = await getDoc(doc(messageRef, mailId));

    if (snapshot.exists()) {
      const data = snapshot.data();
      setReceivedMessage(data.email);
    }
  };
    ...
    <div>
      ...
      >
        <Box sx={style}>
          ...
          <Typography variant="body1" sx={{ marginTop: "1vw", fontSize: "1.2vw", whiteSpace: "pre-wrap" }}>
